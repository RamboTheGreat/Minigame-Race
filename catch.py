...
def catch(screen, WIDTH, HEIGHT, clock, timer1, mode):
	...
	fallingobj1 = catchobjects.Catchobjects(WIDTH, 10, random.randint(0, WIDTH), random.randint(0,1))
	...
	draw.info(screen, WIDTH, HEIGHT, 'Catch 10 to Move on', 100)  # Displays the game info
	...
	while runnning:
		...
		if (random.randrange(0, 100) < 0.75):
			...
			newfallingobj = catchobjects.Catchobjects(WIDTH, random.randint(10,15), random.randint(0, WIDTH), random.randint(0,2))
			all_falling.add(newfallingobj)
		if (random.randrange(0,100) < .10):
			newbomb = catchobjects.Bomb(WIDTH, random.randint(10,20), random.randint(0, WIDTH))
			all_bombs.add(newbomb)
		# Calls the collision function to detects collision
		collisions(all_falling, all_bombs, all_players, player1, screen, WIDTH, HEIGHT)
