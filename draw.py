...
def drawEnd(screen, WIDTH, HEIGHT, timer1):
	...
	time.sleep(1.5)
	win_font = pygame.font.SysFont('Comic Sans MS', 100)
	win_message = win_font.render(str(round(timer1.get_elapsed(),2)), False, colors.white)
	...
	screen.fill(colors.green)
	screen.blit(win1_message, ((WIDTH-win1_width)/2, (HEIGHT- win1_height)/2- win_height))
	screen.blit(win_message, ((WIDTH-win_width)/2, (HEIGHT - win_height)/2))
...
def drawtime(timer1, screen):
	...
	time_message = time_font.render(str(round(timer1.get_elapsed(), 2)), False, colors.white)
	time_width, time_height = time_font.size(str(round(timer1.get_elapsed(), 2)))
	screen.blit(time_message, (3, 0))

def displayscore(screen, WIDTH, HEIGHT, score):
	score_font = pygame.font.SysFont('PressStart2P', 25)
	score_message = score_font.render('Score: ' + str(score), False, colors.white)
...
def welcomeScreen(WIDTH, HEIGHT, screen):
	...
	screen.blit(welcome1_message, ((WIDTH-welcome1_width)/2, (HEIGHT - welcome1_height)/2 - welcome1_height))
	welcome2_message = welcome_font.render('Minigame Race', False, colors.white)
	welcome2_width, welcome2_height = welcome_font.size('Minigame Race')
	screen.blit(welcome2_message, ((WIDTH-welcome2_width)/2, (HEIGHT - welcome2_height)/2 + welcome2_height))
	pygame.display.flip()
	time.sleep(2)
...
def drawremainingtime(start, screen):
	time_font = pygame.font.SysFont('PressStart2P', 25)
	time_message = time_font.render(str(round(start/60, 2)), False, colors.white)
